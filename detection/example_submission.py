...
from zest.compare_with_mntd import zest_compare
# import logging
...
class MetaNetwork(nn.Module):
    def __init__(self, num_queries, num_classes=1):
        ...
        self.mnist = torchvision.datasets.MNIST(download=True, root = '../data/', transform = transform_to_tensor)
        self.gtsrb = torchvision.datasets.GTSRB(download=True, root = '../data/', transform = gtsrb_transform)
        self.cifar10 = torchvision.datasets.CIFAR10(download=True, root = '../data/', transform = transform_to_tensor)
        self.cifar100 = torchvision.datasets.CIFAR100(download=True, root = '../data/', transform = transform_to_tensor)

        self.mnist_train, self.mnist_test = torch.utils.data.random_split(self.mnist, [50000, 10000])
        self.gtsrb_train, self.gtsrb_test = torch.utils.data.random_split(self.gtsrb, [len(self.gtsrb) - 7357, 7357])
        self.cifar10_train, self.cifar10_test = torch.utils.data.random_split(self.cifar10, [len(self.cifar10) - 10000, 10000])
        self.cifar100_train, self.cifar100_test = torch.utils.data.random_split(self.cifar100, [len(self.cifar100) - 10000, 10000])

        self.mnist_dataloader = torch.utils.data.DataLoader(self.mnist, batch_size = num_queries, shuffle = True, drop_last = True)
        self.gtsrb_dataloader = torch.utils.data.DataLoader(self.gtsrb, batch_size = num_queries, shuffle = True, drop_last = True)
        self.cifar10_dataloader = torch.utils.data.DataLoader(self.cifar10, batch_size = num_queries, shuffle = True, drop_last = True)
        self.cifar100_dataloader = torch.utils.data.DataLoader(self.cifar100, batch_size = num_queries, shuffle = True, drop_last = True)
    ...
    def forward(self, net, data_source, dist):
        ...
        if data_source != "MNIST":
            ...
            out = self.layer7(out)
            out = self.final_net(torch.cat((out, torch.unsqueeze(self.flattener(dist), dim = 0)), dim = 1))
            # out = self.final_net(torch.unsqueeze(self.flattener(dist), dim = 0))            
        return out
...
scheduler = torch.optim.lr_scheduler.StepLR(optimizer, 2, gamma=0.2, verbose = True)
...
for epoch in range(num_epochs):
        ...
        for i, (net, label, data_source) in enumerate(pbar):
            if data_source != ['MNIST']:
                ...
                if data_source == 'CIFAR-10':
                    train_fn_given_model = train.TrainFn(batch_size=1, dataset = data_source, architecture=net, loaded_trainset = meta_network.cifar10_train, loaded_testset = meta_network.cifar10_test)
                elif data_source == 'CIFAR-100':
                    train_fn_given_model = train.TrainFn(batch_size=1, dataset = data_source, architecture=net, loaded_trainset = meta_network.cifar100_train, loaded_testset = meta_network.cifar100_test)
                elif data_source == 'GTSRB':
                    train_fn_given_model = train.TrainFn(batch_size=1, dataset = data_source, architecture=net, loaded_trainset = meta_network.gtsrb_train, loaded_testset = meta_network.gtsrb_test)
                distance_list = []
                ...
                print(f"distance from clean_models: {np.array(utils.parameter_distance(clean_masks[data_source], a, order=dist, lime=True))}")
                distance_list.append(np.array(utils.parameter_distance(trojan_masks[data_source], a, order=dist, lime=True)))
                print(f"distance from trojan_models: {np.array(utils.parameter_distance(trojan_masks[data_source], a, order=dist, lime=True))}")
                out = meta_network(net[0], data_source, torch.tensor(np.array(distance_list)).float().to('cuda:0')).to('cuda:0')
                print(f"out: {out}")
                print(f"label: {label}")
                loss = F.binary_cross_entropy_with_logits(torch.unsqueeze(out, 0), torch.nn.functional.one_hot(torch.FloatTensor([label]).to(torch.int64).cuda(), num_classes = 2).float())
                optimizer.zero_grad()
        ...
        scheduler.step()
        torch.save(meta_network, f'latest_net_new.pt')
    ...
    for i, (net, label, data_source) in enumerate(tqdm(loader)):
        ...
        with torch.no_grad():
            if data_source != ['MNIST']:
                ...
                distance_list.append(np.array(utils.parameter_distance(trojan_masks[data_source], a, order=dist, lime=True)))
                print(f"distance from trojan_models: {np.array(utils.parameter_distance(trojan_masks[data_source], a, order=dist, lime=True))}")
                out_1 = meta_network(net[0], data_source, torch.tensor(np.array(distance_list)).float().to('cuda:0')).to('cuda:0')
                print(f"data_source: {data_source}")
                print(f"out: {out_1}")
                print(f"label: {label}")
                loss = F.binary_cross_entropy_with_logits(torch.unsqueeze(out_1, 0), torch.nn.functional.one_hot(torch.FloatTensor([label]).to(torch.int64).cuda(), num_classes = 2).float())
                # correct = int((out.squeeze() > 0).item() == label)
                correct = int((torch.argmax(out_1.squeeze()).item() == torch.argmax(torch.nn.functional.one_hot(torch.FloatTensor([label]).to(torch.int64).cuda(), num_classes = 2).squeeze()).item()))
                loss_list.append(loss.item())
                correct_list.append(correct)
                confusion_matrix[int((torch.argmax(out_1.squeeze()).item() == 1)), label] += 1
                all_scores.append(torch.max(out_1).squeeze().item())
...
#                 distance_list.append(np.array(utils.parameter_distance(clean_masks[data_source], a, order=dist, lime=True)))
#                 distance_list.append(np.array(utils.parameter_distance(trojan_masks[data_source], a, order=dist, lime=True)))
#                 out = meta_network(net, data_source, torch.tensor(np.array(distance_list)).float().to('cuda:0')).to('cuda:0')
                
#             # out = meta_network(net[0], data_source[0], dist)
